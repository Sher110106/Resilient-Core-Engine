...
async fn main() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        match server_clone.accept().await {
            Ok(conn) => {
                println!("   Server: ✅ Connection accepted from {}", conn.remote_address());
                conn
            }
            Err(e) => {
    ...
    println!("   ⚡ Throughput: {:.2} chunks/sec", 10.0 / duration.as_secs_f64());
    ...
    println!("   Total bytes received: {} bytes", server_stats.total_bytes_received);
    ...
    let start = std::time::Instant::now();
    client.send_with_retry(&retry_conn, &retry_chunk, 3).await.unwrap();
    let duration = start.elapsed();
    ...
    for (i, path) in paths.iter().enumerate() {
        println!("      Path {}: {} -> {}", i + 1, path.local_addr, path.remote_addr);
    ...
    if let Some(path) = paths.first() {
        ...
        mp_manager.update_path_metrics(&path.path_id, degraded_metrics).await;
    ...
    println!("   Total bytes transferred: {} bytes ({:.2} KB)", total_bytes, total_bytes as f64 / 1024.0);
    println!("   Average chunk size: {:.2} bytes", total_bytes as f64 / total_chunks as f64);
