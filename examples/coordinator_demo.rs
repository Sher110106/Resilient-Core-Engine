...
async fn main() {
    ...
    println!("Created test file: {} ({} bytes)", file_path1.display(), test_data.len());
    
    let session_id1 = coordinator.send_file(file_path1.clone(), Priority::Normal).await.unwrap();
    ...
    match coordinator.get_progress(&session_id1).await {
        Ok(progress) => {
            ...
            println!("   Chunks: {}/{}", progress.completed_chunks, progress.total_chunks);
            println!("   Bytes: {}/{}", progress.bytes_transferred, progress.total_bytes);
        }
        Err(e) => {
    ...
    let session_id2 = coordinator.send_file(
        test_file2.path().to_path_buf(),
        Priority::High
    ).await.unwrap();
    
    let session_id3 = coordinator.send_file(
        test_file3.path().to_path_buf(),
        Priority::Critical
    ).await.unwrap();
    ...
    let session_id4 = coordinator.send_file(
        test_file4.path().to_path_buf(),
        Priority::Normal
    ).await.unwrap();
