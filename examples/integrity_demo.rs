...
use chunkstream_pro::integrity::{IntegrityVerifier, IntegrityCheck};
use bytes::Bytes;
...
async fn main() {
    ...
    println!("\nSame data produces same checksum: {}", checksum1 == checksum2);
    
    let data3 = b"Different data";
    let checksum3 = IntegrityVerifier::calculate_checksum(data3);
    println!("Different data produces different checksum: {}", checksum1 != checksum3);
    ...
    println!("Sequence: {}/{}", chunk.metadata.sequence_number + 1, chunk.metadata.total_chunks);
    ...
    println!("Original checksum: {}", hex::encode(IntegrityVerifier::calculate_checksum(&corrupted_chunk.data)));
    println!("Stored checksum: {}", hex::encode(corrupted_chunk.metadata.checksum));
    ...
    println!("  Actual: {}", hex::encode(&result1.actual.as_ref().unwrap()[..8]));
    
    println!("\nBad Chunk:");
    println!("  Success: {}", result2.success);
    println!("  Expected: {}", hex::encode(&result2.expected[..8]));
    println!("  Actual: {}", hex::encode(&result2.actual.as_ref().unwrap()[..8]));
    ...
    println!("  ⚡ Throughput: {:.2} chunks/sec", chunk_count as f64 / duration.as_secs_f64());
