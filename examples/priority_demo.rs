...
async fn main() {
    ...
    queue.enqueue(create_test_chunk(Priority::Normal, 1, "Normal priority data")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Critical, 2, "Critical priority data")).unwrap();
    queue.enqueue(create_test_chunk(Priority::High, 3, "High priority data")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Normal, 4, "Another normal")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Critical, 5, "Another critical")).unwrap();
    
    println!("Enqueued 5 chunks:");
    println!("   Normal: {} chunks", queue.pending_count(Priority::Normal));
    println!("   High: {} chunks", queue.pending_count(Priority::High));
    println!("   Critical: {} chunks", queue.pending_count(Priority::Critical));
    ...
    for i in 1..=5 {
        ...
        println!("   {}. Priority: {:?}, Chunk ID: {}, File: {}", 
    ...
    queue.enqueue(create_test_chunk(Priority::Normal, 10, "Seq 10")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Normal, 5, "Seq 5")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Normal, 15, "Seq 15")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Normal, 3, "Seq 3")).unwrap();
    ...
    for i in 0..10 {
        ...
        queue.enqueue(create_test_chunk(priority, i, &format!("Data {}", i))).unwrap();
    }
    ...
    queue.enqueue(create_test_chunk(Priority::Normal, 0, "Normal only")).unwrap();
    
    let allocation1 = queue.allocate_bandwidth(10_000_000);
    println!("Scenario: Only Normal priority chunks present");
    println!("   Critical: {} bps ({:.1} Mbps)", allocation1.critical_bps, allocation1.critical_bps as f64 / 1_000_000.0);
    println!("   High: {} bps ({:.1} Mbps)", allocation1.high_bps, allocation1.high_bps as f64 / 1_000_000.0);
    println!("   Normal: {} bps ({:.1} Mbps) - BOOSTED!", allocation1.normal_bps, allocation1.normal_bps as f64 / 1_000_000.0);
    
    // Add critical chunks
    queue.enqueue(create_test_chunk(Priority::Critical, 1, "Critical added")).unwrap();
    
    let allocation2 = queue.allocate_bandwidth(10_000_000);
    println!("\nScenario: Critical chunks added");
    println!("   Critical: {} bps ({:.1} Mbps) - ACTIVE!", allocation2.critical_bps, allocation2.critical_bps as f64 / 1_000_000.0);
    println!("   High: {} bps ({:.1} Mbps)", allocation2.high_bps, allocation2.high_bps as f64 / 1_000_000.0);
    println!("   Normal: {} bps ({:.1} Mbps)", allocation2.normal_bps, allocation2.normal_bps as f64 / 1_000_000.0);
    ...
    for i in 0..25 {
        queue.enqueue(create_test_chunk(Priority::Normal, i, &format!("Chunk {}", i))).unwrap();
    }
    ...
    for i in 0..5 {
        small_queue.enqueue(create_test_chunk(Priority::Normal, i, "data")).unwrap();
    }
    ...
    queue.clear();
    queue.enqueue(create_test_chunk(Priority::High, 1, "High prio")).unwrap();
    queue.enqueue(create_test_chunk(Priority::Normal, 2, "Normal prio")).unwrap();
    ...
    for i in 0..chunk_count {
        ...
        queue.enqueue(create_test_chunk(priority, i, "data")).unwrap();
    }
    ...
    println!("   Enqueue rate: {:.0} chunks/sec", chunk_count as f64 / enqueue_time.as_secs_f64());
    println!("   Dequeue time: {:?}", dequeue_time);
    println!("   Dequeue rate: {:.0} chunks/sec", dequeued as f64 / dequeue_time.as_secs_f64());
