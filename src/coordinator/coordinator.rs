...
impl TransferCoordinator {
    ...
    pub async fn send_file(
        ...
        let (manifest, chunks) = self.chunk_manager.split_file(&file_path, file_id.clone(), priority).await?;
        ...
        self.session_store.update_status(&session_id, SessionStatus::Active).await?;
        ...
        // Register transfer
        self.active_transfers.insert(session_id.clone(), state_machine.clone());
        self.recent_transfers.insert(session_id.clone(), state_machine);
        ...
        tokio::spawn(async move {
            if let Err(e) = coordinator.transfer_worker(worker_session_id.clone(), manifest, chunks, receiver_addr).await {
    ...
    pub async fn resume_transfer(&self, session_id: &str) -> CoordinatorResult<()> {
        ...
        let state_machine = if let Some(sm) = self.active_transfers.get(session_id) {
            ...
            self.active_transfers.insert(session_id.to_string(), sm.clone());
            sm
        };
        ...
        tokio::spawn(async move {
            if let Err(e) = coordinator.transfer_worker(session_id_str.clone(), manifest, vec![], None).await {
    ...
    pub async fn pause_transfer(&self, session_id: &str) -> CoordinatorResult<()> {
        let state_machine = self.active_transfers.get(session_id)
            .ok_or_else(|| CoordinatorError::TransferNotFound(session_id.to_string()))?;
        
        state_machine.send_event(TransferEvent::Pause)?;
        self.session_store.update_status(session_id, SessionStatus::Paused).await?;
    ...
    pub async fn cancel_transfer(&self, session_id: &str) -> CoordinatorResult<()> {
        ...
        self.session_store.update_status(session_id, SessionStatus::Failed("Cancelled by user".into())).await?;
    ...
    pub fn get_state(&self, session_id: &str) -> Option<TransferState> {
        self.active_transfers.get(session_id).map(|sm| sm.current_state())
    }

    /// List active transfers
    pub fn list_active(&self) -> Vec<String> {
        self.active_transfers.iter().map(|e| e.key().clone()).collect()
    }
    
    /// List all recent transfers (including completed/failed)
    pub fn list_recent(&self) -> Vec<String> {
        self.recent_transfers.iter().map(|e| e.key().clone()).collect()
    }
        ...
        while !chunks_to_transfer.is_empty() {
            ...
            match self.queue.dequeue() {
                Ok(chunk) => {
                    ...
                    // Actually send chunk over network (if receiver address provided)
                        ...
                        match self.transport.connect(addr).await {
                            Ok(conn) => {
                                ...
                                if let Err(e) = self.transport.send_with_retry(&conn, &chunk, 3).await {
                                    ...
                                    self.session_store.mark_chunk_failed(&session_id, chunk_num).await?;
                                    continue;
                                }
                            }
                            Err(e) => {
                                ...
                                // Mark as failed
                                self.session_store.mark_chunk_failed(&session_id, chunk_num).await?;
                                // Network failure - trigger state transition
                    ...
                    // Mark as completed
                    self.session_store.mark_chunk_completed(&session_id, chunk_num).await?;
                    
                    // Update state
                    let progress = self.session_store.load(&session_id).await?
                        .map(|s| s.progress_percent())
                        .unwrap_or(0.0);
                    
                    state_machine.transition(TransferEvent::ChunkCompleted { chunk_number: chunk_num })?;
...
mod tests {
    ...
    async fn test_send_file() {
        ...
        let session_id = coordinator.send_file(file_path, Priority::Normal, None).await.unwrap();
    ...
    async fn test_duplicate_transfer() {
        ...
        let file_path = temp_file.path().to_path_buf();
        coordinator.send_file(file_path.clone(), Priority::Normal, None).await.unwrap();
        
        // Try to send same file again
        let result = coordinator.send_file(file_path, Priority::Normal, None).await;
