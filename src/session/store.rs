    ...
    pub async fn load(&self, session_id: &str) -> SessionResult<Option<SessionState>> {
        ...
        if let Some(row) = row {
            ...
            let state = SessionState {
                ...
                completed_chunks: serde_json::from_str(
                    &row.try_get::<String, _>("completed_chunks")?,
                )?,
...
mod tests {
    ...
    async fn test_update_status() {
        ...
        store
            .update_status("test-session", SessionStatus::Paused)
            .await
            .unwrap();
    ...
    async fn test_list_sessions() {
        ...
        for i in 0..5 {
            let manifest = create_test_manifest();
            let state =
                SessionState::new(format!("session-{}", i), "test-file".to_string(), manifest);
            store.save(&state).await.unwrap();
        }
